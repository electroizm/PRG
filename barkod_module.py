...
def _normalize_stok_kod(stok_kod: str) -> str:
    ...
    fatura_okumalari: her zaman 10 haneli base kod (orn: '3200418840')"""
    s = str(stok_kod or '')
    dash_idx = s.find('-')
    if dash_idx > 0:
        return s[:dash_idx]
...
class SupabaseClient:
    ...
    def get_all_invoices(self, limit=2000) -> list:
        url = f"{self.rest_url}/satis_faturasi"
        all_data = []
        page_size = 500
        offset = 0
    ...
    def get_readings_by_invoices(self, fatura_no_list: list) -> list:
        """fatura_okumalari tablosundan okuma kayitlarini al"""
        ...
        for i in range(0, len(fatura_no_list), batch_size):
            ...
            url = f"{self.rest_url}/fatura_okumalari"
            params = {
    ...
    def get_all_evrakno_sira(self) -> set:
        """Supabase'deki tum benzersiz evrakno_sira degerlerini al"""
        url = f"{self.rest_url}/satis_faturasi"
        params = {
            'select': 'evrakno_sira',
            'limit': '10000'
...
class LoadDataThread(QThread):
    ...
    def run(self):
        try:
            all_data = self.supabase_client.get_all_invoices(limit=2000)
            ...
            try:
                ...
                for r in readings:
                    fno = str(r.get('fatura_no', ''))
                    sk = _normalize_stok_kod(r.get('stok_kod', ''))
                    ps = int(r.get('paket_sira', 0) or 0)
                    qr = str(r.get('qr_kod', ''))

                    group_key = (fno, sk)
                    if group_key not in readings_map:
                        readings_map[group_key] = {}
                    ...
                    if ps not in readings_map[group_key]:
                        readings_map[group_key][ps] = []
                    readings_map[group_key][ps].append(info)
...
class SyncThread(QThread):
    ...
    def run(self):
        try:
            # 1. Supabase'den son evrakno_sira al
            self.progress_updated.emit(5, "Supabase'den son evrak sira aliniyor...")
            son_evrak_sira = self.supabase_client.get_max_evrakno_sira()
            logger.info(f"Supabase'deki son evrak sira: {son_evrak_sira}")

            if self.is_cancelled:
                return

            # 2. Mikro SQL Server'a baglan ve yeni faturalari cek
            self.progress_updated.emit(15, "Mikro SQL Server'a baglaniliyor...")
            faturalar = self._fetch_from_mikro(son_evrak_sira)
            ...
            if not faturalar:
                self.sync_finished.emit({
                    ...
                    'evrak_sayisi': 0,
                    'toplam': 0,
                    'mesaj': "Mikro'dan aktarilmamis Satis Faturasi bulunamadi."
                })
                return

            self.progress_updated.emit(40, f"{len(faturalar)} yeni kayit bulundu")

            # 3. Benzersiz product code'lari topla
            product_codes = list(set(
                row['sth_stok_kod'][:10]
                for row in faturalar
                if row.get('sth_stok_kod')
            ))

            # 4. Dogtas API'den paket bilgisi al
            paket_bilgileri = {}
            ...
            # 7. Iptal edilen faturalari temizle
            silinen = 0
            if not self.is_cancelled:
                self.progress_updated.emit(96, "Iptal edilen faturalar kontrol ediliyor...")
                try:
                    silinen = self._cleanup_cancelled_invoices()
                    if silinen > 0:
    ...
    def _fetch_from_mikro(self, son_evrak_sira: int) -> list:
        ...
        logger.info(f"Mikro SQL Server'a baglaniliyor: {server}/{database}")

        with pyodbc.connect(conn_str, timeout=30) as conn:
            cursor = conn.cursor()
            cursor.execute(MIKRO_SQL_QUERY, son_evrak_sira)
            columns = [col[0] for col in cursor.description]
            rows = cursor.fetchall()

        logger.info(f"Mikro'dan {len(rows)} kayit alindi")
        return [dict(zip(columns, row)) for row in rows]

    def _cleanup_cancelled_invoices(self) -> int:
        """Mikro'da silinmis (iptal edilmis) faturalari Supabase'den temizle"""
        # Mikro'daki aktif evrakno_sira'lari al
        active_sira = self._fetch_active_evrakno_sira()
        if not active_sira:
            return 0

        # Supabase'deki tum evrakno_sira'lari al
        supabase_sira = self.supabase_client.get_all_evrakno_sira()

        # Mikro'da olmayan Supabase kayitlarini bul
        to_delete = supabase_sira - active_sira

        if to_delete:
            logger.info(f"Iptal edilmis {len(to_delete)} fatura bulundu: {to_delete}")
            self.supabase_client.delete_by_evrakno_sira_list(list(to_delete))

        return len(to_delete)

    def _fetch_active_evrakno_sira(self) -> set:
        """Mikro'daki aktif satis faturasi evrakno_sira degerlerini al"""
        server = os.getenv('SQL_SERVER', '')
        database = os.getenv('SQL_DATABASE', '')
        username = os.getenv('SQL_USERNAME', '')
        password = os.getenv('SQL_PASSWORD', '')
        ...
        query = """
            ...
            WHERE sth_evraktip = 4 AND sth_tarih > '2026-01-01'
        """
...
class SatisTeslimatWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.all_data = []
        self.filtered_data = []
        # readings_map: (fatura_no, stok_kod) -> {paket_sira: [{'type','user','time'}, ...]}
    ...
    def load_invoice_table(self):
        ...
        self.status_label.setText("Faturalar yukleniyor...")
        self.refresh_button.setEnabled(False)

        self._load_thread = LoadDataThread(self.supabase_client)
    ...
    def _get_row_colors(self, row_data):
        ...
        colors = set()
        fatura_no = str(row_data.get('evrakno_sira', ''))
        stok_kod = _normalize_stok_kod(row_data.get('stok_kod', ''))
        depo_no = str(row_data.get('cikis_depo_no', '') or '')
        paket_readings = self.readings_map.get((fatura_no, stok_kod), {})
    ...
    def apply_filters(self):
        ...
        if tarih_text:
            filtered = [r for r in filtered
                        if tarih_text in str(r.get('tarih', ''))]
    ...
    def populate_table(self):
        ...
        try:
            ...
            for i, row_data in enumerate(self.filtered_data):
                ...
                # miktar = kac adet urun, paket_sayisi = her urunun kac paketi var
                # Gosterim: miktar grup x paket_sayisi etiket, gruplar | ile ayrilir
                if okuma_col_idx is not None:
                    ...
                    # Bu satirin okuma kayitlari: {paket_sira: [{'type','user','time'}, ...]}
                    paket_readings = self.readings_map.get((fatura_no, stok_kod), {})
                    ...
                    for unit_idx in range(miktar):
                        ...
                        for p in range(1, paket + 1):
                            # Bu paket_sira icin okuma listesi
                            ...
                            # Okunmus paketlere tooltip ekle
                            if info:
                                tip = f"{info['user']}\n{info['time']}"
